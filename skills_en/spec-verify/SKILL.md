...
### Step 1: Choose Verification Path

Default to read-only verification in the current session. If the user explicitly requests sub-agents or the active Codex instructions allow delegation, split this into the `explorer` or `qa` role.
...
Verification prompt:
  ...
  Write results to SPEC-REVIEW.md or summarize them to the user.
```
