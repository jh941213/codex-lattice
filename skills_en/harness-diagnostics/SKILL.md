...
|---|-----------|-------------|
| 1 | **Agent Entry Point** | AGENTS.md existence + clear agent entry point |
...
| 5 | **Progressive Disclosure** | Information layering (AGENTS.md -> rules/ -> docs/ -> code) |
...
1. Explore project root with read-only commands such as `rg`, `find`, and `ls`
...
2. Detect drift:
   - AGENTS.md content vs actual project structure mismatch
...
- Split long analysis into a sub-agent only when appropriate; default to fast read-only checks
